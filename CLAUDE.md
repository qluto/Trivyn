...
pkill -f Trivyn      # Kill running instance
```
...
Trivyn is a **multiplatform desktop app** (macOS, Windows, Linux) built with **Tauri 2.x** implementing the "Three Wins" productivity method. Users set 3 goals each for daily, weekly, and monthly timeframes.
...
- **MenuBarPopover** (`src/components/popover/`): Popup window from tray icon
  - `MenuBarPopover.tsx`: Main container with Trivyn branding and bottom navigation
