...
fn get_screen_visible_frame<R: Runtime>(window: &tauri::WebviewWindow<R>) -> Option<(i32, i32, i32, i32)> {
    ...
    unsafe {
        ...
        println!("DEBUG: full_frame: origin=({}, {}), size=({}, {})",
            full_frame.origin.x, full_frame.origin.y, full_frame.size.width, full_frame.size.height);
        println!("DEBUG: visible_frame: origin=({}, {}), size=({}, {})",
        ...
        println!("DEBUG: Menu bar height: {}", screen_top);
        println!("DEBUG: Dock height: {}", visible_frame.origin.y);
        println!("DEBUG: Calculated bounds: left={}, top={}, right={}, bottom={}",
...
fn constrain_window_to_screen<R: Runtime>(window: &tauri::WebviewWindow<R>) -> tauri::Result<()> {
    ...
    let (screen_left, screen_top, screen_right, screen_bottom) = {
        ...
        println!("DEBUG: Tauri monitor: position=({}, {}), size=({}, {})",
        ...
        if let Some(ns_bounds) = get_screen_visible_frame(window) {
            ...
            println!("DEBUG: Calculated scale factor: {}", scale_factor);
            ...
            println!("DEBUG: Scaled NSScreen bounds: left={}, top={}, right={}, bottom={}",
                scaled_bounds.0, scaled_bounds.1, scaled_bounds.2, scaled_bounds.3);
            scaled_bounds
        } else {
            // Fallback to monitor API with estimated margins for menu bar and Dock
            println!("DEBUG: Using Tauri monitor bounds (fallback)");
    ...
    println!("DEBUG: Window position=({}, {}), size=({}, {})", position.x, position.y, size.width, size.height);
    println!("DEBUG: Window bounds: left={}, top={}, right={}, bottom={}", window_left, window_top, window_right, window_bottom);
    println!("DEBUG: Screen bounds: left={}, top={}, right={}, bottom={}", screen_left, screen_top, screen_right, screen_bottom);
    ...
    if window_right > screen_right {
        println!("DEBUG: Window right ({}) > screen right ({}), adjusting", window_right, screen_right);
        new_x = screen_right - size.width as i32;
        needs_adjustment = true;
    }
    if window_left < screen_left {
        println!("DEBUG: Window left ({}) < screen left ({}), adjusting", window_left, screen_left);
        new_x = screen_left;
        needs_adjustment = true;
    }
    ...
    if window_bottom > screen_bottom {
        println!("DEBUG: Window bottom ({}) > screen bottom ({}), adjusting", window_bottom, screen_bottom);
        new_y = screen_bottom - size.height as i32;
        needs_adjustment = true;
    }
    if window_top < screen_top {
        println!("DEBUG: Window top ({}) < screen top ({}), adjusting", window_top, screen_top);
        new_y = screen_top;
        needs_adjustment = true;
    }
    ...
    if needs_adjustment {
        println!("DEBUG: Adjusting window position from ({}, {}) to ({}, {})", position.x, position.y, new_x, new_y);
